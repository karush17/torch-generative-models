...
class NCSNTrainer(BaseTrainer, metaclass=abc.ABCMeta):
    def __init__(self, args, env, buffer, filename):
        ...
        self.score = ScoreConvNet(self.args, self.state_shape, self.action_dim).to(device)
        self.optimizer = torch.optim.Adam(self.score.parameters(), lr=self.args.lr, \
    ...
    def train(self, batch):
        ...
        scaled_score = self.score(states)
    ...
    def ssm_loss(self, dats, n_particles=1):
        samples = dats.unsqueeze(0).expand(n_particles, *dats.shape).contiguous().view(-1, *dats.shape[1:])
    ...
    def sample(self, init_samples=[], sgld_steps=None):
        self.score.eval()
        sgld_lr = 0.00002
        if init_samples==[]:
