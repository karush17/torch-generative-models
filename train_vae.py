"""Implements the main VAE experiment."""
...
from algorithms.vae import VAETrainer
...
def train():
    ...
    algo = VAETrainer(args, env, buffer, filename)
    if os.path.isfile(filename+'model.pt') and args.load_model:
        algo.load()

    for step in tqdm.tqdm(range(0, args.vae_epochs),
                                smoothing=0.1,
        ...
        if step % args.log_freq == 0:
            ...
            samples = algo.sample_vae(init_samples=init, sgld_steps=5*args.sgld_steps)
