"""Implements the main EBMVAE experiment."""
...
from algorithms.ebmvae_v2 import EBMVAETrainer
...
def train():
    ...
    algo = EBMVAETrainer(args, env, buffer, filename)
    if os.path.isfile(filename+'model.pt') and args.load_model:
        algo.load()

    for step in tqdm.tqdm(range(0, args.vae_epochs+args.ebm_epochs),
                                smoothing=0.1,
        ...
        if step % args.log_freq == 0:
            ...
            samples, _ = algo.sample_latent(init_samples=init,
                                            sgld_steps=2*args.sgld_steps)
